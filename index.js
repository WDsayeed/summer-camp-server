...
async function run() {
  try {
    ...
    const usersCollection = client.db('globalLanguage').collection('users')
    const classAddCollection = client.db('globalLanguage').collection('classAdd')
    const paymentsCollection = client.db('globalLanguage').collection('payments')
    ...
    app.get('/classadd', verifyJWT, async(req, res)=>{
      ...
      const query = {email: email}
      const result = await classAddCollection.find(query).toArray()
      res.send(result)
    })
    ...
    app.delete('/classadd/:id', async(req,res)=>{
      const id = req.params.id 
      const query = {_id: new ObjectId(id)}
      const result = await classAddCollection.deleteOne(query)
      res.send(result)
    })
